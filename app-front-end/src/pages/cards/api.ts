...
export default class ExportAPI {
  static getUserSpotifyTokenValidity(userId: number = 1): Promise<ApiResult<string>> {
    return apiRequest(`/user/spotify-token-validity/${userId}`);
  }

  static getPlaylists(): Promise<ApiResult<PlaylistResultDTO>> {
    return apiRequest('/playlists');
  }

  static getCardData(playlist_ids: string[]): Promise<
    ApiResult<{
      valid: SongCardDTO[];
      invalid: SongCardDTO[];
    }>
  ...
  static exportCardData(card_data: SongCardDTO[]): Promise<ApiResult<string>> {
